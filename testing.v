...
module object (
    ...
    // precompute sums (Quartus doesn't allow slicing expressions)
    wire [9:0] sum10  = top_h_next10 + GAP;        // top height + GAP
    wire [9:0] ydim10 = 10'd480 - sum10;           // bottom pillar height
    ...
    wire wrap_load = (fsm_q==4'h8) && (X=='d0);         // state S8
    ...
    always @(posedge Clock) begin
        if (Resetn==0) begin
            if (TOP) begin
                ...
                y_base_reg <= sum10[nY-1:0];                        // start after gap
                ydim_reg   <= ydim10[nY-1:0];                       // to bottom
            end
            ...
            if (TOP) begin
                ...
                y_base_reg <= sum10[nY-1:0];
                ydim_reg   <= ydim10[nY-1:0];
            end
        end
    end
